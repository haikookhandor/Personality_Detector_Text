...
class Big5():
    def __init__(self):
        self.df = pd.read_csv('data/BIG5/data.csv', sep='\t') # Read data
        self.preprocess_df() # Preprocess data
        self.questions_key = { # Questions to traits given by the dataset
    ...
    def handle_personality_test(self, answers): # answers come from the frontend (yet to be implemented!)
        answer_dict = {} # Initialize answer dictionary
        # key is a question, value is its answer
        for question, answer in answers.items():
            key = self.questions_key[question]
            answer_dict[key] = answer

        score_dict = {'O_score': 0, 'C_score': 0, 'E_score': 0, 'A_score': 0, 'N_score': 0} # Initialize score dictionary
        for trait_key, answer in answer_dict.items(): # Calculate score for each trait
            if 'O' in trait_key:
        ...
        for key, score in score_dict.items():
            score_dict[key] = score/10 # Normalize score

        perc_dict = {} # Initialize percentile dictionary
        for key, score in score_dict.items(): # Calculate percentile for each trait
            if key == 'O_score':
    ...
    def calc_score(self, df): # Calculate score for each trait
        score = []
        for row in df.values:
            score.append(row.mean())
        return score

    def preprocess_df(self): # All possible value of each trait
